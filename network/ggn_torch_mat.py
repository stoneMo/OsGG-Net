...
class GraphGenNetwork(nn.Module):
    ...
    def __init__(self, num_node=68, center_landmark=32, strategy='uniform', max_hop=1, dilation=1):
        ...
        self.node_selflink = torch.from_numpy(np.array([[i, i] for i in range(self.num_node)]).T)
    ...
    def get_adjacency(self, hop_dis, strategy):
        ...
        # print("hop_dis[0]:", hop_dis[0])

        batch_size = hop_dis.shape[0]

        valid_hop = range(0, self.max_hop + 1, self.dilation)
        adjacency = Variable(torch.zeros((batch_size, self.num_node, self.num_node)), requires_grad=True)
        # print("valid_hop:", valid_hop)

        # print("adjacency:", adjacency.shape)

        for hop in valid_hop:
            # print("hop:", hop)
            adjacency.data[hop_dis == hop] = 1

        # print("adjacency[0]:", adjacency[0])
        
        normalize_adjacency = normalize_digraph(adjacency)

        if strategy == 'uniform':
            A = torch.zeros((batch_size, 1, self.num_node, self.num_node))
    ...
    def forward(self, x):
        ...
        node_selflink = self.node_selflink.unsqueeze(0).repeat(batch_size, 1, 1)   # (12, 2, 19)

        # print("node_selflink:", node_selflink.shape)

        # for i in range(batch_size):

            # edge = x[i].numpy()       #(2, 68)
 
            # print("edge:", edge.shape)                                   # (2, 19)
            # print("self.node_selflink:", self.node_selflink.shape)       # (2, 19)
            # edge = np.concatenate([edge, self.node_selflink], axis=1)
 
        edge = torch.cat([x, node_selflink], dim=-1)         # (12, 2, 19+19)
        # print("edge:", edge.shape)

        hop_dis = get_hop_distance(self.num_node, edge, max_hop=self.max_hop)
        # print("hop_dis:", hop_dis.shape)

        A_face_graph, adjacency_face_graph = self.get_adjacency(hop_dis, self.strategy)
...
def get_hop_distance(num_node, edge, max_hop=1):
    ...
    # edge: (batch_size, 2, 12)

    batch_size = edge.shape[0]

    A = torch.zeros((batch_size, num_node, num_node))
    ...
    for i in range(batch_size):
        A[i, edge[i,0], edge[i,1]] = 1
        A[i, edge[i,1], edge[i,0]] = 1

    # compute hop steps
    # hop_dis = np.zeros((num_node, num_node)) + np.inf

    hop_dis = torch.ones_like(A) * float("Inf")
    # print("hop_dis:", hop_dis.shape)

    # print("hop_dis[0]:", hop_dis[0])
    # transfer_mat = torch.zeros((len(range(max_hop + 1)), num_node, num_node))
    ...
    # print("arrive_mat:", arrive_mat)

    for d in range(max_hop, -1, -1):
        # print("d:", d)
        # print("arrive_mat[d]:", arrive_mat[d])
        # hop_dis[arrive_mat[d]] = d
        # print("transfer_mat[d][0]:", transfer_mat[d][0])

        hop_dis[transfer_mat[d]>0] = d
...
def normalize_digraph(A):
    ...
    Dl = torch.sum(A, dim=1)

    Dn = torch.zeros((batch_size, num_node, num_node))
    ...
    for j in range(batch_size):
        # for i in range(num_node):
            # if Dl[j,i] > 0:
        Dn[j, Dl[j]>0, Dl[j]>0] = Dl[j,Dl[j]>0]**(-1)
    ...
    # Dn[Dl>0, Dl>0] = Dl[Dl>0]**(-1)
    # Dn[Dl > 0, Dl > 0] = Dl **(-1)

    
    # print("A:", A.shape)
    # print("Dn:", Dn.shape)
    AD = torch.matmul(A, Dn)
    return AD
...
if __name__ == "__main__":
    ...
    adjacency_matrix = adjacency_face_graph[0].detach().cpu().numpy()
